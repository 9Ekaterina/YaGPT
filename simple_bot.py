...
import telebot
from config import TOKEN, MAX_SESSIONS, MAX_TOKENS_IN_SESSION
import sql_engine
import gptyandex
...
def start_user(message):
    ...
    if current_session > int(MAX_SESSIONS):
        print("ВЫ привысили количество допустимых сессий")
    else:
        print(f"У вас осталось {MAX_SESSIONS-current_session}")
        #считаем токены
